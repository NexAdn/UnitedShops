...
public class TradeManager {
    ...
    public boolean tradeOfferItemForMoney(Player player, ItemStack offer, double want) {
        return this.tradeOfferItemForMoney(player, offer, want, player.getInventory());
    }

    public boolean tradeOfferItemForMoney(Player player, ItemStack offer, double want, Inventory source) {
        if (!this.initialized) {
    ...
    public boolean tradeOfferMoneyForItem(Player player, double offer, ItemStack want) {
        return this.tradeOfferMoneyForItem(player, offer, want, player.getInventory());
    }

    public boolean tradeOfferMoneyForItem(Player player, double offer, ItemStack want, Inventory source) {
        if (!this.initialized) {
            this.plugin.sendMessage(player, this.plugin.getL10n().getMessage("transactionFailed").str());
            return false;
        }
        ...
        EconomyResponse response = null;
        Inventory transactionInv = source;
